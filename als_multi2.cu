		...
		const int ITERS, const int X_BATCH, const int THETA_BATCH)
{
	...
	for(int iter = 0; iter < ITERS ; iter ++){
		...
		cusparsecall (cusparseScsrmm2(cushandle, CUSPARSE_OPERATION_NON_TRANSPOSE,
				CUSPARSE_OPERATION_TRANSPOSE, m, f, n, nnz, &alpha, descr, csrVal[0],
				csrRowIndex[0], csrColIndex[0], thetaT[0], f, &beta, ytheta, m) );
		...
		cublascall(cublasSgeam(handle, CUBLAS_OP_T, CUBLAS_OP_N, f, m, &alpha,
				(const float * ) ytheta, m, &beta, ythetaT[0], f, ythetaT[0], f));
		//cudaDeviceSynchronize();
		//cudaCheckError();
		...
		// copy input to all devices
		for (int device = 1; device < nDevices; ++device) {
			cudacall(cudaMemcpyPeerAsync(ythetaT[device], device, ythetaT[0], 0, f * m * sizeof(ythetaT[0][0]), stream[0]));
		}
		...
		for(int batch_id_base = 0; batch_id_base < X_BATCH; batch_id_base += nDevices){
			...
			for (int device = 0; device < nDevices; ++device) {
				...
				if (batch_id < X_BATCH) {
					...
					if(f == 100){
						get_hermitian100<<<batch_size, 64, SCAN_BATCH * f/2 * sizeof(float2), stream[device]>>>
						...
						get_hermitianT10<<<batch_size, block_dim, SCAN_BATCH * f/2 * sizeof(float2), stream[device]>>>
							(batch_offset, tt[device], csrRowIndex[device], csrColIndex[device], lambda, m, f, thetaT[device]);
					}

					updateXWithCGHostAsync(tt[device], &XT[device][batch_offset*f], &ythetaT[device][batch_offset*f], batch_size, f, CG_ITER, &stream[device]);

					for (int j = 0; j < nDevices; ++j) {
						if (j != device) {
							cudacall(cudaMemcpyPeerAsync(&XT[j][batch_offset*f], j, &XT[device][batch_offset*f], device, batch_size * f * sizeof(XT[0][0]), stream[device]));
						}
					}
...
///*
		...
		for (int device = 1; device < nDevices; ++device) {
			cudacall(cudaMemcpyPeerAsync(yTXT[device], device, yTXT[0], 0, f * n * sizeof(yTXT[0][0]), stream[0]));
		}
		cudaStreamSynchronize(stream[0]);
			...
			for (int device = 0; device < nDevices; ++device) {
				...
				if (batch_id < THETA_BATCH) {
					...
					if(f == 100){
						...
						get_hermitianT10<<<batch_size, block_dim, SCAN_BATCH*f*sizeof(float), stream[device]>>>
							(batch_offset, xx[device], cscColIndex[device], cscRowIndex[device], lambda, n, f, XT[device]);
					}

					printf("\tCG solver with fp32.\n");
					updateXWithCGHostAsync(xx[device], &thetaT[device][batch_offset*f], &yTXT[device][batch_offset*f], batch_size, f, CG_ITER, &stream[device]);


					for (int j = 0; j < nDevices; ++j) {
						if (j != device) {
							cudacall(cudaMemcpyPeerAsync(&thetaT[j][batch_offset*f], j, &thetaT[device][batch_offset*f], device, batch_size * f * sizeof(thetaT[0][0]), stream[device]));
						}
					}
		...
		for (int device = 0; device < nDevices; ++device) {
			...
			RMSE_reduction2<<<error_size_train, 256>>>
					(offset, csrVal[device], cooRowIndex[device], csrColIndex[device], thetaT[device], XT[device], errors_train[device], nnz, f);

			reduction<<<1, 1024>>>(errors_train[device], rmse_train_device[device], error_size_train);

			offset = nnz_test_device * device;

			RMSE_reduction2<<<error_size_test, 256>>>
			...
			reduction<<<1, 1024>>>(errors_test[device], rmse_test_device[device], error_size_test);
		...
		for (int device = 0; device < nDevices; ++device) {
			...
			cudacall(cudaMemcpy(rmse_train_host + device, rmse_train_device[device], sizeof(rmse_train_host[0]), cudaMemcpyDeviceToHost));
			cudacall(cudaMemcpy(rmse_test_host + device, rmse_test_device[device], sizeof(rmse_test_host[0]), cudaMemcpyDeviceToHost));
